...
router.get("/onetask/:id", async (req, res) => {
  ...
  // console.log(id);
  const out = await pool.query(
    'select uid, taskid, task, subject, "desc", deadline, completed, "precentComp", subid, startdate FROM public."Task" where taskid = $1;',
    [id]
  );
...
router.get("/predefined", async (req, res) => {
  const out = await pool.query(
    'SELECT uid, taskid, task, subject, "desc", deadline, completed, "precentComp", subid, startdate FROM public.predefinedtask',
    []
  );
  res.send(out.rows);
});
...
router.get("/archived", async (req, res) => {
  const out = await pool.query(
    'SELECT uid, taskid, task, subject, "desc", deadline, completed, "precentComp", subid, startdate FROM public.archivedtask',
    []
  );
  res.send(out.rows);
});
